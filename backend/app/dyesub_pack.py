...
def _paste_placement(
    ...
    resized = art.resize((tw, th), Image.Resampling.LANCZOS)
...
def render_piece_png(
    ...
    *,
    ...
    cut_yd = _y_down_poly(piece.get("cutMm") or [], height_mm)
    sew_yd = _y_down_poly(piece.get("sewMm") or [], height_mm) if piece.get("sewMm") else []
...
def build_dyesub_zip(
    *,
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        nest_png, nest_pos = nest_pieces(rendered, roll_width_in=roll, dpi=min(dpi, 150), gap_in=0.35)
        z.writestr("NEST/roll-44in.png", nest_png)
        z.writestr("NEST/layout.json", json.dumps({"rollWidthIn": roll, "dpi": min(dpi, 150), "positions": nest_pos}, indent=2))
        ...
        z.writestr(
            ...
            "NEST/roll-44in.png    Nested on 44 in Epson SureColor F6200 roll (150 DPI preview/layout).\n"
