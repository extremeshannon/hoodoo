...
router = APIRouter(prefix="/dyesub", tags=["dyesub"], dependencies=[Depends(get_current_user)])
...
@router.post("/jobs/{job_id}/quote", response_model=DyeSubJobOut)
def request_quote(
    job_id: UUID,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
...
def download_pack(
    ...
    try:
        ...
        logger.exception("Print pack failed for job %s", job_id)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Print pack failed: {e}",
        ) from e
