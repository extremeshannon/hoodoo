"""Screen-print configurator API: blanks, saved jobs, art upload, quote, shop pack."""
...
from app.models import Order, OrderLine, ScreenPrintArt, ScreenPrintJob, User
from app.screenprint_catalog import get_garment, list_garments

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/screenprint", tags=["screenprint"], dependencies=[Depends(get_current_user)])
...
class ScreenPrintJobCreate(BaseModel):
    garment_id: str
    name: str = Field("Untitled screen print", max_length=255)
    layout: dict[str, Any] = Field(default_factory=dict)
    notes: str | None = None


class ScreenPrintJobUpdate(BaseModel):
...
class ScreenPrintEstimateIn(BaseModel):
    garment_id: str
    quantity: int = 1
    colors: int = 1
    locations: int = 1
...
class ScreenPrintArtOut(BaseModel):
    id: UUID
    filename: str
    mime: str
    byte_size: int
...
class ScreenPrintJobOut(BaseModel):
    id: UUID
    garment_id: str
    name: str
    status: str
    ...
    artworks: list[ScreenPrintArtOut] = Field(default_factory=list)
...
def _job_out(job: ScreenPrintJob, *, include_owner: bool = False) -> ScreenPrintJobOut:
    arts = [
        ScreenPrintArtOut(
            id=a.id,
            filename=a.filename,
...
def _get_job(db: Session, job_id: UUID, user: User) -> ScreenPrintJob:
    job = db.scalar(
        select(ScreenPrintJob)
        .where(ScreenPrintJob.id == job_id)
        .options(selectinload(ScreenPrintJob.artworks), selectinload(ScreenPrintJob.user))
    )
    if not job:
...
def _estimate_for_job(root: Path, job: ScreenPrintJob) -> dict[str, Any]:
    ...
    layout = job.layout or {}
    colors, locations = _layout_counts(layout)
    ...
    label = f"{garment.get('vendor')} {garment.get('model')} {garment.get('name')}"
    if color_row:
        label = f"{label} · {color_row.get('name')}"
    return estimate(
        {
            "method": "screen",
            "garment": "tee" if garment.get("category") in ("tshirt", "tank", "longsleeve", "polo") else (
                "hoodie" if garment.get("category") in ("hoodie", "crew") else "jersey"
            ),
            "garmentName": label,
            "blankPrice": float(garment.get("price") or 12),
            "quantity": qty,
            "colors": colors,
            "locations": locations,
        }
...
def quote_estimate(body: ScreenPrintEstimateIn):
    root = _repo_root()
    g = get_garment(root, body.garment_id)
    if not g:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unknown garment")
    label = body.garment_name or f"{g.get('vendor')} {g.get('model')} {g.get('name')}"
    blank = body.blank_price if body.blank_price is not None else float(g.get("price") or 12)
    return estimate(
        {
            "method": "screen",
            "garment": "tee",
...
@router.get("/jobs", response_model=list[ScreenPrintJobOut])
def list_jobs(
    ...
    q = (
        select(ScreenPrintJob)
        .options(selectinload(ScreenPrintJob.artworks), selectinload(ScreenPrintJob.user))
        .order_by(ScreenPrintJob.updated_at.desc())
    )
    include_owner = False
    if all_users:
        ...
        q = q.where(ScreenPrintJob.user_id == user.id)
    jobs = db.scalars(q).unique().all()
    return [_job_out(j, include_owner=include_owner) for j in jobs]


@router.post("/jobs", response_model=ScreenPrintJobOut, status_code=status.HTTP_201_CREATED)
def create_job(
    body: ScreenPrintJobCreate,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    root = _repo_root()
    ...
    job = ScreenPrintJob(
        ...
        name=(body.name or "Untitled screen print").strip() or "Untitled screen print",
...
@router.get("/jobs/{job_id}", response_model=ScreenPrintJobOut)
def get_job(
    job_id: UUID,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
...
@router.put("/jobs/{job_id}", response_model=ScreenPrintJobOut)
def update_job(
    job_id: UUID,
    body: ScreenPrintJobUpdate,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@router.post("/jobs/{job_id}/art", response_model=ScreenPrintArtOut, status_code=status.HTTP_201_CREATED)
...
@router.post("/jobs/{job_id}/quote", response_model=ScreenPrintJobOut)
def request_quote(
    job_id: UUID,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    ...
    if order is None:
        ...
        db.add(
            OrderLine(
                order_id=order.id,
                product_id=None,
                product_slug_snapshot="screen-print",
                ...
                unit_price=each,
                line_total=subtotal,
                label_snapshot=f"Screen print · {job.name} · {qty} pc",
            )
        )
        ...
        if order.lines:
            ...
            line.label_snapshot = f"Screen print · {job.name} · {qty} pc"
            line.configuration = {
                "kind": "screen-print",
                "jobId": str(job.id),
                "garmentId": job.garment_id,
...
def download_pack(
    ...
    filename = f"hoodoo-screenprint-{safe_job}-{str(job.id)[:8]}.zip"
    return Response(
        content=raw,
